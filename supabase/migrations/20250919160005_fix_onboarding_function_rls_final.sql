-- Migration: Final fix for onboarding function RLS issues
-- The core issue is that SECURITY DEFINER functions don't maintain user context for RLS

-- Drop the previous wrapper function as it's not the right approach
DROP FUNCTION IF EXISTS public.complete_onboarding_transaction_wrapper(UUID, JSONB, JSONB, JSONB, BOOLEAN, BOOLEAN);

-- The real solution is to modify the original function to use SECURITY INVOKER
-- This means it runs with the caller's privileges, maintaining auth context for RLS
...
BEGIN
    ...
    IF auth.uid() != p_user_id THEN
        RAISE EXCEPTION 'Unauthorized: authenticated user does not match provided user ID';
    END IF;
    ...
    BEGIN
        ...
        -- Check what columns exist in fbr_profiles and create accordingly
        IF EXISTS (
        ...
        IF NOT EXISTS (SELECT 1 FROM fbr_profiles WHERE user_id = p_user_id) THEN
            RAISE EXCEPTION 'Failed to create FBR profile - RLS may be blocking access';
        END IF;
