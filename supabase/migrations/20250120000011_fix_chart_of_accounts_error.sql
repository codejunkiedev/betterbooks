...
-- Create the corrected function that handles missing chart_of_accounts table
...
DECLARE
    ...
    v_combination JSONB;
    v_chart_of_accounts_exists BOOLEAN;
BEGIN
    ...
    ) INTO v_chart_of_accounts_exists;
    ...
    BEGIN
        ...
        -- Create FBR profile with primary business activity (fbr_profiles has updated_at column)
        ...
        -- Handle opening balance if provided and chart_of_accounts table exists
        v_has_opening_balance := FALSE;
        IF NOT p_skip_balance AND p_opening_balance IS NOT NULL AND v_chart_of_accounts_exists THEN
            ...
            IF v_opening_amount IS NOT NULL AND v_opening_date IS NOT NULL THEN
                ...
                IF v_cash_account_id IS NOT NULL AND v_equity_account_id IS NOT NULL THEN
                    -- Create journal entry for opening balance (journal_entries table doesn't have updated_at column)
                    ...
                    -- Create journal entry lines (journal_entry_lines table doesn't have updated_at column)
        ...
        -- Assign accountant if available (skip if company_accountants table doesn't exist)
        ...
        -- Note: Skipping company_accountants insertion as table may not exist
        -- This can be added later when the table is created

        -- Create activity log (activity_logs table doesn't have updated_at column)
        INSERT INTO activity_logs (
            ...
            jsonb_build_object(
                ...
                'chart_of_accounts_available', v_chart_of_accounts_exists
            ),
            NOW()
        );
...
-- Add comment
COMMENT ON FUNCTION complete_onboarding_transaction IS 'Completes user onboarding with company creation, FBR profile setup, and support for both new business activity/sector structure and legacy structure - handles missing chart_of_accounts table gracefully';
