-- Migration: Recreate complete_onboarding_transaction function with new implementation
-- Drops existing function and creates new one with activities/sectors array structure
...
-- Create new function with updated implementation
    ...
    BEGIN
        ...
        IF v_has_opening_balance AND v_opening_amount IS NOT NULL AND v_opening_amount > 0 THEN
            ...
            IF v_cash_account_id IS NOT NULL AND v_equity_account_id IS NOT NULL THEN
                -- Create journal entry for opening balance using correct schema
...
-- Add comment
COMMENT ON FUNCTION public.complete_onboarding_transaction IS 'Complete user onboarding process with activities and sectors arrays structure';
...
BEGIN
    ...
    IF function_count = 1 THEN
        RAISE NOTICE 'SUCCESS: complete_onboarding_transaction function recreated with new activities/sectors arrays structure';
    ELSE
        RAISE EXCEPTION 'ERROR: Function recreation failed. Found % functions', function_count;
    END IF;
END $$;