-- Migration: Update complete_onboarding_transaction function to handle missing updated_at columns
-- This fixes the "column updated_at does not exist" errors
...
BEGIN
    ...
    BEGIN
        ...
        -- Create company record (avoid updated_at if column doesn't exist)
        ...
        -- Create FBR profile record with activities and sectors (avoid updated_at if column doesn't exist)
        ...
        IF v_has_opening_balance AND v_opening_amount IS NOT NULL AND v_opening_amount > 0 THEN
            IF EXISTS (
                ...
                -- Get cash account ID (look for account_id = '1000' or cash in name)
                ...
                -- Get equity account ID (look for account_id = '3000' or equity type)
                ...
                IF v_cash_account_id IS NOT NULL AND v_equity_account_id IS NOT NULL THEN
                    -- Create journal entry for opening balance using correct schema (only if tables exist)
                    IF EXISTS (
                        SELECT 1 FROM information_schema.tables
                        ...
                        -- Create journal entry lines using correct schema (type and amount columns)
                        IF EXISTS (
                            SELECT 1 FROM information_schema.tables
        ...
        -- Update user onboarding status (only if auth.users exists and has raw_user_meta_data)
        IF EXISTS (
            SELECT 1 FROM information_schema.tables
...
-- Add comment
COMMENT ON FUNCTION public.complete_onboarding_transaction(UUID, JSONB, JSONB, JSONB, BOOLEAN, BOOLEAN) IS 'Completes user onboarding with company creation, FBR profile setup - fixed for column compatibility';