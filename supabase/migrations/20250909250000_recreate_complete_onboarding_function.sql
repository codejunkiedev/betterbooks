...
-- First, let's verify current function state
DO $$
DECLARE
...
BEGIN
    ...
    RAISE NOTICE 'Current complete_onboarding_transaction function count: %', function_count;
END $$;

-- Drop any existing function signatures to ensure clean replacement
...
BEGIN
    ...
    BEGIN
        ...
        -- Set up chart of accounts for the company
...
-- Add comment
COMMENT ON FUNCTION public.complete_onboarding_transaction IS 'Complete user onboarding process with company setup, FBR profile, and business activities';
...
BEGIN
    ...
    IF function_count = 1 THEN
        RAISE NOTICE 'SUCCESS: complete_onboarding_transaction function created successfully';
    ELSE
        RAISE EXCEPTION 'ERROR: Function creation failed. Found % functions', function_count;
    END IF;
END $$;
