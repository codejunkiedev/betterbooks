...
-- Create updated function without any updated_at column references that might cause trigger issues
...
BEGIN
    ...
    -- Start transaction
    BEGIN
        -- Create company (without updated_at column)
        ...
        -- Create FBR profile with primary business activity (without updated_at to avoid trigger issues)
        ...
        IF NOT p_skip_balance AND p_opening_balance IS NOT NULL THEN
            ...
            IF v_opening_amount IS NOT NULL AND v_opening_date IS NOT NULL THEN
                ...
                IF v_cash_account_id IS NOT NULL AND v_equity_account_id IS NOT NULL THEN
                    -- Create journal entry for opening balance (without updated_at to avoid trigger issues)
                    ...
                    -- Create journal entry lines (without updated_at to avoid trigger issues)
        ...
        -- Assign accountant if available (without updated_at to avoid trigger issues)
