...
export default function Onboarding() {
  ...
  const validateFbrProfile = (data: CompanySetupData) => {
    return !!(
      ...
      data.fbr_activities &&
      data.fbr_activities.length > 0 &&
      data.fbr_sectors &&
      data.fbr_sectors.length > 0
    );
  };
    ...
    try {
      ...
      const payload: OnboardingPayload = {
        ...
        fbr_data: {
          ...
          activities: formData.fbr_activities,
          sectors: formData.fbr_sectors,
        },
        opening_balance: formData.skip_balance
          ? null
          : { amount: parseFloat(formData.cash_balance) || 0, date: formData.balance_date },
      ...
      toast({ title: "Success", description: "Company setup completed successfully!", variant: "default" });
      ...
      const errorInfo =
        error instanceof Error
          ...
          : {
              title: "Setup Failed",
              message:
                "We encountered an issue while setting up your company. Please try again or contact support if the problem persists.",
            };
      ...
      case 5:
        return (
          <ReviewStep
            ...
            fbrActivities={formData.fbr_activities}
            fbrSectors={formData.fbr_sectors}
          />
        );
      default:
        return null;
      ...
      <div className="bg-white shadow-sm border-b">
        <div className="max-w-4xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="text-sm text-gray-500">Welcome! Let's get you set up</div>
          </div>
        </div>
      </div>
      ...
      <div className="py-8">
        <div className="max-w-4xl mx-auto px-4">
          ...
          <NavigationButtons
            ...
            canProceed={
              currentStep === 1
                ? !!(formData.company_name && formData.company_type)
                : currentStep === 2
                ? validateOpeningBalance(formData.cash_balance, formData.balance_date, formData.skip_balance)
                : currentStep === 3
                ? validateTaxInformation(
                    formData.tax_id_number,
                    formData.filing_status,
                    formData.tax_year_end,
                    formData.skip_tax_info
                  )
                : currentStep === 4
                ? validateFbrProfile(formData)
                : true
            }
          />
        </div>
