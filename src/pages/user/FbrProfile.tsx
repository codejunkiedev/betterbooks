...
export default function FbrProfileNew({
    ...
    useEffect(() => {
        const loadAvailableSectors = async () => {
            if (businessActivitySelection.business_activity_type_ids.length > 0) {
                try {
                    const { data } = await getAvailableSectorsForActivities(businessActivitySelection.business_activity_type_ids);
    ...
    }, [businessActivitySelection.business_activity_type_ids, toast]);

    // Load applicable scenarios when both business activities and sectors are selected
    useEffect(() => {
        const loadApplicableScenarios = async () => {
            if (businessActivitySelection.business_activity_type_ids.length > 0 && businessActivitySelection.sector_ids.length > 0) {
                try {
                    ...
                    const scenarios = (data || []).map((item: { scenario_id: number }) => ({
                        scenario_id: item.scenario_id || 0,
                        scenario_code: `SN${(item.scenario_id || 0).toString().padStart(3, '0')}`,
                        business_activity: 'Combined',
                        sector: 'Multiple'
                    }));
    ...
    }, [businessActivitySelection.business_activity_type_ids, businessActivitySelection.sector_ids, toast]);
    ...
    const errors = useMemo(() => {
        ...
        if (businessActivitySelection.business_activity_type_ids.length === 0) errs.business_activities = "At least one business activity is required";
        if (businessActivitySelection.business_activity_type_ids.length > 0 && businessActivitySelection.sector_ids.length === 0) {
            errs.sectors = "At least one sector is required when business activities are selected";
        }
        if (businessActivitySelection.combinations.length === 0 && businessActivitySelection.business_activity_type_ids.length > 0 && businessActivitySelection.sector_ids.length > 0) {
                            ...
                            <div className="space-y-2 p-4 border rounded-lg bg-gray-50">
                                {availableSectors.length > 0 ? (
                                    availableSectors.map((sector, index) => (
                                        <div key={`sector-${sector.sector_id}-${index}`} className="flex items-center space-x-2 p-2 rounded hover:bg-gray-100 transition-colors">
                                            <Checkbox
                                                id={`sector-${sector.sector_id}-${index}`}
                                            ...
                                            />
                                            <label htmlFor={`sector-${sector.sector_id}-${index}`} className="text-sm font-medium cursor-pointer flex-1">
                                                {sector.sector_name}
                                            </label>
                    ...
                    {enrichedCombinations.length > 0 && (
                        <div>
                            <label className="block text-sm font-medium mb-1">
                                Selected Combinations
                                <span className="text-xs text-gray-500 ml-2">
                                    ({enrichedCombinations.length} combinations)
                                </span>
                            </label>
                            <div className="space-y-2 max-h-40 overflow-y-auto">
                                {enrichedCombinations.map((combination, index) => (
                                    <div key={`combination-${combination.business_activity_type_id}-${combination.sector_id}-${index}`} className="flex items-center justify-between p-3 border rounded-lg bg-white shadow-sm">
                        ...
                        <div>
                            ...
                            <div className="space-y-2 max-h-40 overflow-y-auto">
                                {applicableScenarios.map((scenario, index) => (
                                    <div key={`scenario-${scenario.scenario_id}-${index}`} className="flex items-center justify-between p-3 border rounded-lg bg-blue-50 shadow-sm">
                                        <div className="flex items-center space-x-2">
