...
export async function getFbrProfileByUser(userId: string): Promise<FbrProfile | null> {
  const { data, error } = await supabase.from("fbr_profiles").select("*").eq("user_id", userId).single();
...
export async function upsertFbrProfile(payload: FbrProfilePayload) {
  ...
  if (existingCnicNtn) {
    throw new Error(
      `CNIC/NTN ${payload.cnic_ntn} is already registered by another user. Please use a different CNIC/NTN or contact support if this is an error.`
    );
  }
 ...
 */
export async function updateFbrConnectionStatus(
  userId: string,
  environment: "sandbox" | "production",
  status: "connected" | "failed"
): Promise<FbrConfigStatus> {
  try {
...
export async function saveFbrCredentials(userId: string, sandboxKey?: string, productionKey?: string): Promise<void> {
  ...
  if (existingRecord) {
    // Update existing record
    const { error } = await supabase.from("fbr_api_configs").update(updatePayload).eq("user_id", userId);
    ...
    // Insert new record
    const { error } = await supabase.from("fbr_api_configs").insert({
      user_id: userId,
      ...updatePayload,
    });
...
export async function getFbrConfigStatus(userId: string): Promise<FbrConfigStatus> {
  const { data, error } = await supabase
    .from("fbr_api_configs")
    .select(
      "sandbox_status, production_status, last_sandbox_test, last_production_test, sandbox_api_key, production_api_key"
    )
    .eq("user_id", userId)
    .single();
...
export async function getProvinceCodes() {
  const { data, error } = await supabase.from("province_codes").select("*").order("state_province_code");
...
export async function getBusinessActivities() {
  const { data, error } = await supabase.from("business_activity").select("*").order("sr");
...
export async function getBusinessActivityTypes() {
  const { data, error } = await supabase.from("business_activity_types").select("*").order("name");
 ...
 */
export async function getScenariosForCombinations(businessActivityTypeIds: number[], sectorIds: number[]) {
  const { data, error } = await supabase
 ...
 */
export async function getScenariosForBusinessActivityAndSector(businessActivityTypeIds: number[], sectorIds: number[]) {
  const { data, error } = await supabase.rpc("get_scenarios_for_business_activity_and_sector", {
...
export async function getUserPrimaryBusinessActivity(userId: string) {
  const { data, error } = await supabase.rpc("get_user_primary_business_activity", { p_user_id: userId });
...
export async function getUserBusinessActivities(userId: string) {
  const { data, error } = await supabase.rpc("get_user_business_activities", { p_user_id: userId });
...
export async function addUserBusinessActivity(
  ...
  const { data, error } = await supabase.rpc("add_user_business_activity", {
    p_user_id: userId,
 ...
 */
export async function removeUserBusinessActivity(userId: string, businessActivityTypeId: number) {
  const { data, error } = await supabase.rpc("remove_user_business_activity", {
    p_user_id: userId,
    p_business_activity_type_id: businessActivityTypeId,
  });
 ...
 */
export async function setPrimaryBusinessActivity(userId: string, businessActivityTypeId: number) {
  const { data, error } = await supabase.rpc("set_primary_business_activity", {
    p_user_id: userId,
    p_business_activity_type_id: businessActivityTypeId,
  });
 ...
 */
export async function getMandatoryScenarios(businessActivityId: number): Promise<
  Array<{
    scenario_id: string;
    scenario: {
      code: string;
...
export async function initializeScenarioProgress(userId: string, businessActivityId: number): Promise<void> {
  try {
    ...
    const { error } = await supabase.from("fbr_scenario_progress").insert(progressEntries);
 ...
 */
export async function updateScenarioProgressForNewBusinessActivity(
  userId: string,
  newBusinessActivityId: number
): Promise<void> {
  try {
    // First, delete all existing progress entries for this user
    const { error: deleteError } = await supabase.from("fbr_scenario_progress").delete().eq("user_id", userId);
...
export async function getFilteredMandatoryScenarios(
  ...
  const progressMap = new Map<
    string,
    Pick<FbrScenarioProgress, "scenario_id" | "status" | "attempts" | "last_attempt" | "completion_timestamp">
  >();
...
export async function getFilteredMandatoryScenariosForUser(
  ...
  const sectorIds = [...new Set(userActivities.map((ua) => ua.sector_id).filter(Boolean))];
 ...
 */
export async function getScenarioById(scenarioId: string): Promise<FbrScenario | null> {
  try {
...
/**
 * Add a successfully completed scenario to user's FBR profile
 */
export async function addSuccessfulScenario(userId: string, scenarioId: string): Promise<void> {
  try {
    const { data: currentProfile, error: fetchError } = await supabase
      .from("fbr_profiles")
      .select("successful_scenarios")
      .eq("user_id", userId)
      .single();

    if (fetchError) {
      if (fetchError.code === "PGRST116") {
        console.warn(`No FBR profile found for user ${userId}`);
        return;
      }
      throw fetchError;
    }

    const currentScenarios = currentProfile.successful_scenarios || [];

    if (currentScenarios.includes(scenarioId)) {
      console.log(`Scenario ${scenarioId} already marked as successful for user ${userId}`);
      return;
    }
    ...
    const { error: updateError } = await supabase
      .from("fbr_profiles")
      .update({
        successful_scenarios: updatedScenarios,
        updated_at: new Date().toISOString(),
      })
      .eq("user_id", userId);
    ...
    console.log(`Successfully added scenario ${scenarioId} to user ${userId}'s successful scenarios`);
  } catch (error) {
...
/**
 * Get user's successful scenarios from FBR profile
 */
export async function getUserSuccessfulScenarios(userId: string): Promise<string[]> {
  try {
    const { data, error } = await supabase
      .from("fbr_profiles")
      .select("successful_scenarios")
    ...
    return data.successful_scenarios || [];
  } catch (error) {
    console.error("Error getting successful scenarios:", error);
    throw error;
  }
}
...
export async function cleanupScenarioProgressData(userId: string): Promise<void> {
  try {
    ...
    const nonNumericRecords =
      progressRecords?.filter(
        (record) => !/^\d+$/.test(record.scenario_id) // Non-numeric values
      ) || [];
      ...
      try {
        ...
        if (scenarioError) {
          if (scenarioError.code === "PGRST116") {
            ...
            await supabase.from("fbr_scenario_progress").delete().eq("id", record.id);
            continue;
          }
          throw scenarioError;
        }
        ...
        if (existingCorrectRecord) {
          ...
          console.log(
            `Correct record exists for ID ${scenarioData.id}, deleting incorrect record ${record.scenario_id}`
          );
          await supabase.from("fbr_scenario_progress").delete().eq("id", record.id);
        } else {
          // Update the record to use the correct scenario ID
          console.log(`Updating record from ${record.scenario_id} to ${scenarioData.id}`);
          await supabase.from("fbr_scenario_progress").update({ scenario_id: scenarioData.id }).eq("id", record.id);
        }
      } catch (error) {
