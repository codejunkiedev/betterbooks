...
import type {
  FbrSandboxTestRequest,
  FbrSandboxTestResponse,
  SaleTypeToRateResponse,
  SroScheduleResponse,
  SroScheduleParams,
  SroItemResponse,
  SroItemParams,
...
const FBR_DATA_ENDPOINTS = {
  ...
  sro_item: "https://gw.fbr.gov.pk/pdi/v2/SROItem",
} as const;
...
export async function getAllHSCodes(apiKey: string): Promise<ApiResponse<HSCodeSearchResult[]>> {
  try {
    ...
    return {
      success: true,
      message: "All HS codes retrieved successfully",
      data: (response.data as HSCodeSearchResult[]) || [],
    };
  } catch (error) {
    const fbrError = error as FbrApiError;
...
/**
 * Get SRO schedule data from FBR SroSchedule API
 * @param apiKey - FBR API key for authentication
 * @param params - SroSchedule API parameters
 * @returns Promise with SRO schedule details for the given parameters
 */
export async function getSroSchedule(
  apiKey: string,
  params: SroScheduleParams
): Promise<ApiResponse<SroScheduleResponse[]>> {
  try {
    const response = await httpClient.request({
      method: "GET",
      url: FBR_DATA_ENDPOINTS.sro_schedule,
      headers: {
        Authorization: `Bearer ${apiKey}`,
        "Content-Type": "application/json",
      },
    ...
    return {
      success: true,
      message: "SRO schedule data retrieved successfully",
      data: (response.data as SroScheduleResponse[]) || [],
    };
  } catch (error) {
    const fbrError = error as FbrApiError;
    return {
...
/**
 * Get SRO item data from FBR SROItem API
 * @param apiKey - FBR API key for authentication
 * @param params - SROItem API parameters
 * @returns Promise with SRO item details for the given parameters
 */
export async function getSroItem(apiKey: string, params: SroItemParams): Promise<ApiResponse<SroItemResponse[]>> {
  try {
    const response = await httpClient.request({
      method: "GET",
      url: FBR_DATA_ENDPOINTS.sro_item,
      headers: {
        Authorization: `Bearer ${apiKey}`,
        "Content-Type": "application/json",
      },
    ...
    return {
      success: true,
      message: "SRO item data retrieved successfully",
      data: (response.data as SroItemResponse[]) || [],
    };
  } catch (error) {
    const fbrError = error as FbrApiError;
    return {
