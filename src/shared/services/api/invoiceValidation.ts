...
function processFBRValidationResponse(fbrResponse: Record<string, unknown>): InvoiceValidationResponse {
  ...
  console.log("Processing FBR response:", {
    ...
    validationResponseKeys: fbrResponse.validationResponse
      ? Object.keys(fbrResponse.validationResponse as Record<string, unknown>)
      : [],
  ...
  if (
    fbrResponse.dated &&
    (!fbrResponse.validationResponse ||
      fbrResponse.validationResponse === null ||
      Object.keys((fbrResponse.validationResponse as Record<string, unknown>) || {}).length === 0)
  ) {
  ...
  if (fbrResponse.validationResponse && fbrResponse.validationResponse !== null) {
    ...
    if (validationResponse.status === "Valid" && validationResponse.invoiceStatuses) {
      ...
      const warnings = invoiceStatuses.filter((status) => status.status === "Valid" && status.statusCode === "00");
  ...
  if (
    fbrResponse.error ||
    fbrResponse.errorCode ||
    (fbrResponse.validationResponse &&
      fbrResponse.validationResponse !== null &&
      (fbrResponse.validationResponse as Record<string, unknown>)?.errorCode)
  ) {
    // Process errors using the FBR error handler
    const errorResults = FBRErrorHandler.processFBRResponse(fbrResponse);
