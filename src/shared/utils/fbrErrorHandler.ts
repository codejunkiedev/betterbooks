...
import { FBR_ERROR_MESSAGES, FBR_ERROR_CATEGORY, getFBRErrorInfo } from "@/shared/constants/fbr";
...
export class FBRErrorHandler {
  ...
  private static processValidationResponse(validationResponse: Record<string, unknown>): ValidationResult[] {
    ...
    if (validationResponse.errorCode && validationResponse.error) {
      ...
      results.push({
        ...
        message: (validationResponse.error as string) || errorInfo.message,
        code: errorCode,
      ...
      (validationResponse.invoiceStatuses as Array<Record<string, unknown>>).forEach((itemStatus) => {
        if (itemStatus.status === "Invalid" || itemStatus.statusCode === "01") {
          ...
          results.push({
            field: `items[${itemSNo}]`,
            severity: errorInfo.severity,
            message: (itemStatus.error as string) || errorInfo.message,
            code: errorCode,
    ...
    errors.forEach((error) => {
      ...
      if (error.code) {
        ...
        if (errorInfo) {
          ...
          if (
            errorInfo.category === FBR_ERROR_CATEGORY.AUTHENTICATION ||
  ...
  static formatErrorMessage(error: ValidationResult): string {
    ...
    let message = error?.message || errorInfo.message;
  ...
  static getResolutionSuggestions(errors: ValidationResult[]): string[] {
    const suggestions: string[] = [];
    const errorCodes = errors.map((error) => error.code as string).filter((code): code is string => !!code);
