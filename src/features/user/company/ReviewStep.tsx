...
import { getCombinationsByActivitiesAndSectors } from "@/shared/constants";
...
export function ReviewStep({
  ...
  const combinations = useMemo(() => {
    return getCombinationsByActivitiesAndSectors(fbrActivities, fbrSectors);
  }, [fbrActivities, fbrSectors]);
  ...
  const getBalanceDisplay = () => {
    ...
    return `$${parseFloat(cashBalance).toLocaleString("en-US", { minimumFractionDigits: 2 })} as of ${new Date(
      balanceDate
  ...
  const getTaxInfoDisplay = () => {
    ...
    if (taxYearEnd)
      parts.push(
        `Tax Year End: ${new Date(taxYearEnd).toLocaleDateString("en-US", {
          year: "numeric",
          month: "long",
          day: "numeric",
        })}`
  ...
  return (
    <Card className="shadow-lg border-0">
      ...
      <CardContent className="space-y-4">
              ...
              {combinations && combinations?.length > 0 && (
                <div className="space-y-2">
                  ...
                  {combinations?.map((combination, index) => (
                    <div
                      key={`review-combination-${index}`}
                      className="flex items-center justify-between bg-white p-2 rounded border"
                    >
                      <div className="flex items-center space-x-2">
                        <span className="text-sm">
                          {combination.businessActivity} - {combination.sector}
                        </span>
                      </div>
                    </div>
                  ))}
                  <div className="text-xs text-gray-500 mt-2">
                    Selected Activities: {combinations?.length || 0} | Selected Sectors: {combinations?.length || 0} |
                    Total Combinations: {combinations?.length || 0}
                  </div>
                </div>
              )}
            </div>
        ...
        <p className="text-gray-600 text-sm">Click below to complete your company setup.</p>
      </CardContent>
    </Card>
  );
}
